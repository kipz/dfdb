...
(defn verify-3hop-path [db node]
  "Check if there's actually a 3-hop path to node"
  (let [;; Find all nodes that connect to this node (1 hop back)
        one-hop-back (query/query db '[:find ?f2 :where [?f2 :connected ?target]]
                                  {:target node})
        ;; For each, check if there's a 2-hop path to it
        two-hop-paths (mapcat
                       (fn [[f2]]
                         (query/query db '[:find ?f1 :where [?f1 :connected ?f2target]]
                                      {:f2target f2}))
                       one-hop-back)
        ;; For each f1, check if there's a 3-hop path
        three-hop-paths (mapcat
                         (fn [[f1]]
                           (query/query db '[:find ?u :where [?u :connected ?f1target]]
                                        {:f1target f1}))
                         two-hop-paths)]
    (seq three-hop-paths)))
  ...
  (let [db (dfdb/create-db {:storage-backend :memory})
    ...
    (when (not= sub-final naive-final)
      (let [spurious (clojure.set/difference sub-final naive-final)]
        (println "MISMATCH! Spurious count:" (count spurious))
        (doseq [[node] (take 2 spurious)]
          (let [has-path (verify-3hop-path db node)]
            (println "Node" node "- Has 3-hop path?" (boolean has-path))
            (when (not has-path)
              (println "  CONFIRMED SPURIOUS - subscription added it but no path exists!"))))))
